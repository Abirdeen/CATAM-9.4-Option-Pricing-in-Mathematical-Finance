...
fn stock_moneyness(call: &EuropeanCall, interest_rate: f64) -> f64 {
    ...
    let moneyness = 1.0/(underlying.std_dev*call.strike_time.sqrt()) * ((underlying.fwd_price/call.strike_price).ln() + (interest_rate + underlying.std_dev.powf(2.0)/2.0)*call.strike_time);
    return moneyness
}
...
pub fn black_scholes_price(call: &EuropeanCall, interest_rate: f64) -> Result<f64,StatsError> {
    ...
    let undiscounted_price = std_norm_cdf(stock_moneyness(call, interest_rate))?*underlying.fwd_price - std_norm_cdf(rfa_moneyness(call, interest_rate))?*call.strike_price;
